...
export const registerUser = async (req, res) => {
    try {
        const { fullname, email, password, faculty, level, gender } = req.body;

        // VALIDATION
        if (!fullname || !email || !password || !faculty || !level || !gender) {
        ...
        user.verifyTokenExpire = Date.now() + 1000 * 60 * 60;
        ...
        try {
            ...
            console.log("Verification email error:", emailErr.message);
...
export const forgotPassword = async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(404).json({
                success: false,
                message: "User found", // Standard practice is often to not reveal if user exists, but following existing flow
            });
        }
        ...
        try {
            ...
            console.log("Reset email error:", emailErr.message);

            return res.status(502).json({
                success: false,
                message: "Reset link could not be sent. Please try again later.",
            });
        }
